angular.module('your_app_name.controllers', [])
    ...
    .controller('SpeedCtrl', function ($scope, $http, $ionicLoading, $q, $timeout, NOKIA_HERE_API_URL) {

        var posOptions = {timeout: 700, enableHighAccuracy: true};
        ...
        // Get location and speed limit for road segment
        (function poll() {
            $timeout(function () {
                geo.getCurrentPosition(success, fail, posOptions); //, {timeout: 10000});
                var URL = NOKIA_HERE_API_URL + $scope.positions.lat + "," + $scope.positions.long + '&linkattributes=sc,sl';
                $http.get(URL)
                    .then(function (response) {
                        var x2js = new X2JS();
                        var responseJson = x2js.xml_str2json(response.data);
                        $scope.positions.currentSegmentSpeedLimit = responseJson.GetLinkInfo.Response.Link.DynamicSpeedInfo.BaseSpeed;
                        $scope.positions.currentSegmentSpeedLimit *= 3.6;
                        $scope.positions.currentSegmentSpeedLimit = $scope.positions.currentSegmentSpeedLimit.toFixed();
                        $scope.positions.currentLocation = responseJson.GetLinkInfo.Response.Link.Address.Street;
                    });
                poll();
            }, 700);
        })();
